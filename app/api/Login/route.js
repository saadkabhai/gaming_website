...
export async function POST(req) {
    ...
    const getuser = Users.find((item) => item.Email == Email)
    if (getuser) {
        const databasepassword = EncryptText.get(getuser.Password)
        if (databasepassword == Password) {
            if (getuser.Status == 'UA') {
...
`
                ...
                const date = new Date();
                await User.updateOne({ Email }, { $set: { OTPCode: OtpCode, OTPCodeUpdate: date } });
                ...
                return new NextResponse(JSON.stringify({
                    message: 'Successful.', Username: getuser.Username, Color: getuser.Color
                }), {
                    status: 200,
                    headers: {
                        'Access-Control-Allow-Origin': allowedOrigin,
