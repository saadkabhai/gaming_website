...
export async function POST(req) {
    ...
    const diffInMs = date - getuser.OTPCodeUpdate;
    ...
    if (valid == true) {
        if (getuser) {
            if (getuser.OTPCode == Code) {
                await User.updateOne({ Email: Email }, { $set: { Status: 'A' } })
...
`
        ...
        await User.updateOne({ Email: Email }, { $set: { OTPCode: OtpCode, OTPCodeUpdate: date } })
        return new NextResponse(JSON.stringify({
